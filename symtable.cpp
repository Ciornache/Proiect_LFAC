...
{
    printf("sunt in addsymbol nume %s type %s\n", name.c_str(), type.c_str());
    if (symTable.find(name) == symTable.end())
    {
        printf("sunt in if add symbol\n");
        symExist[name] = true;
        symTable[name].type = type;
        symTable[name].value = SymTable::getDefaultValue(type);
        return true;
    }
    return false;
}
...
bool SymTable::updateSymbol(std::string name, value val)
{
    ...
    if (std::holds_alternative<int>(val) && symTable[name].type == "int")
        ...
        symTable[name].value = val, ok = 1;
    printf("updateSYmbol returneaza %d \n", ok);
    return ok;
}
...
SymTable::SymTable(SymTable *symTable)
{
    ...
    this->type = symTable->type;
    this->name = symTable->name;
}
